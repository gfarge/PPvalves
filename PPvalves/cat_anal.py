...
""" Module used to analyse activity catalogs, adapted to analyse
...
def event_count(ev_type,states,time,catalog=False,b_x=None):
	...
	if catalog & (type(b_x) == type(None)):
		raise ValueError('When catalog option is turned on, the barriers'' location needs to be specified with the b_x argument.')
	# Get opening time idx for each barrier in one array
	Nt = states.shape[0] # time dimension
...
def event_count_signal(event_time, dt, t0=0., tn='max', norm=False):
	...
	- Parameters
	        + :param event_time: 1D array of event times, in seconds.
	        + :type event_time: ndarray
	        + :param dt: duration of bins, final discretization of the event count signal, in seconds.
	        + :type dt: float
	        + :param t0: float, beginning time of the first bin, in seconds. Default is 0.
	        + :type t0: float
	        + :param tn: float, beginning time of the final bin, in seconds. Default is time of last event.
	        + :type tn: float
	        + :param norm: default is False, option to normalize by norm of vector.
	        + :type norm: bool
	
	- Output
	        + :return:  event_count_signal : 1D array of evenly spaced count of events in bins of dt.
	...
	event_count, bin_edges = np.histogram(event_time,\
	        bins=int((tn-t0)//dt+2),range=(t0,tn+dt))
	
	# Normalize with norm of vector
	if norm == True:
		event_count = event_count.astype(float)/np.linalg.norm(event_count)
	
	# /!\ bin_edges length is 1 unit longer than event_count
	
	return event_count, bin_edges
...
def autocorr(sig,dt,norm=True):
        ...
        - Parameters
                ...
                + :type dt: float
                + :param norm: autocorrelation normalized by the square of the norm of the signal (default is True).
                + :param norm: bool
